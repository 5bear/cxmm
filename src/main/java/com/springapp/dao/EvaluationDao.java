...
public class EvaluationDao extends BaseDao {
    ...
    public List<WxEvaluation>cxList(){
        return this.findAll("select new Map(e.id as id,e.uid as uid,e.name as name,e.time as time,e.phone as phone,e.address as address,e.evaluation_status as evaluation_status) from WxEvaluation as e,WxUser as w where e.uid.id=w.id and w.aid=0",WxEvaluation.class);
    }
    /*代理用户*/
