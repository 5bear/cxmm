...
public class Test2Dao extends BaseDao {
    ...
    public List<Question2> getList(String uid) {//获取多选题
       ...
       return this.findAll("select new Map(q.id as id,q.type as type,q.question as question) from Question2 as q,SChoice as s where s.uid.openid=? and s.cid=q.type group by q.id",Question2.class,new Object[]{uid});
    ...
    public List<LResult>getResult(int uid){
        return this.findAll("from LResult where uid.uid=? order by score desc", LResult.class, new Object[]{uid});
    }
/*
